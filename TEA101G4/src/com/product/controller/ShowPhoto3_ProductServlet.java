...
@WebServlet("/back-end/product/productshow3.do")
public class ShowPhoto3_ProductServlet extends HttpServlet {
	/**
	 * 
	 */
	private static final long serialVersionUID = 3079534928456120382L;
	private static final String SHOW_PHOTO = "SELECT PHOTO3 FROM PRODUCT WHERE PRODUCTID = ?";
			...
			if (ds != null) {
				...
				try {
					if (rs.next()) {
						...
						System.out.println("3run here:--rs.next()_else--");
...
//				e.printStackTrace();
					System.out.println("3.1run here:--IOException--");
					InputStream in = getServletContext()
							.getResourceAsStream("/back-end/product/images/non_default.png");
					byte[] b = new byte[in.available()];
					in.read(b);
